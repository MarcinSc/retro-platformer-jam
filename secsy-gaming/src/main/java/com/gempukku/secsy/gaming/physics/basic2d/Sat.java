...
class Sat {
    ...
    public static Boolean hasCollided(Vector[] poly1, Vector[] poly2, Float maxDist) {
    ...
    private static ArrayList<Vector> polyToEdges(Vector[] poly) {
        ArrayList<Vector> vectors = new ArrayList<Vector>(poly.length);
    ...
    private static Vector project(Vector[] poly, Vector axis) {
        List<Float> dots = new ArrayList<Float>();
